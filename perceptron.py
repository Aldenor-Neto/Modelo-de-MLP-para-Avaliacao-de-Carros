...
class Perceptron(nn.Module):
    def __init__(self, input_size, output_size):
        ...
        self.fc = nn.Linear(input_size, output_size)  # Camada única que conecta entrada à saída
    
    def forward(self, x):
        x = self.fc(x)  # Sem função de ativação intermediária
        return x
...
for i in range(10):
    ...
    with open(f'resultados/resultados_per.txt', 'a') as file:
    ...
    plt.legend()
    plt.savefig(f'curvas/comparacao_tempos_execucao_per_{i+1}.png')
    ...
    plt.savefig(f'matrizes/Matriz_de_confusao_per_{i+1}')